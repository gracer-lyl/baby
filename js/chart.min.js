	...
	var b = navigator.userAgent.toLowerCase(),
		...
		m = {
			...
			u.apply = function(a, b) {
				...
				if (a && u.isObject(b)) {
					for (var e in b) u.isDefined(b[e]) && (u.isObject(b[e]) ? u.isObject(a[e]) ? u.merge(a[e], b[e]) : a[e] = u.clone(b[e], !0) : a[e] = b[e]);
					return u.merge(a, c, d)
				}
				return a
			...
			var v = Math.sin,
				...
				I = {}, J = {}, K = function(a, b) {
					...
					if (/^#(([a-fA-F0-9]{6,7})|([a-fA-F0-9]{3}))$/.exec(a)) return L(a);
					throw new Error("invalid colors value '" + a + "'")
				}, P = function(a, b, c, d) {
				...
				R = .14,
				S = function(a, b) {
					return b = b || R, a > .5 ? b - (1 - a) / 10 : a > .1 ? b - .16 + a / 5 : a > b ? b : a / 2
				}, T = function(a, b, c, d) {
					if (!b) return b;
					b = M(O(b));
					var e = N(b);
					return d = 0 != d ? d || Q : d, e[1] -= d, a ? (e[2] -= S(e[2], c), e[1] = u.upTo(e[1], 1), e[2] = u.lowTo(e[2], 0)) : (e[2] += S(1 - e[2], c), e[1] = u.lowTo(e[1], 0), e[2] = u.upTo(e[2], 1)), P(e, b[3])
				};
			return u.apply(u, {
				...
				uid: function() {
					var a = function() {
						return E(65536 * (1 + Math.random())).toString(16).substring(1)
					};
					return function(b) {
				...
				register: function(a) {
					if (u.isString(a)) J[a.toLowerCase()] = a;
						...
						var b = a.get("id");
						if (b && "" != b || (b = a.push("id", u.uid(a.type))), I[b]) throw new Error("Exist Reduplicate id :" + b);
						a.id = b, I[b] = a
					}
				},
				create: function(a) {
				...
				parseFloat: function(a, b) {
					if (!u.isNumber(a) && (a = H(a), !u.isNumber(a))) throw new Error("[" + b + "]is not a valid number.");
					return a
				},
				ceil: function(a) {
					return K(a, 1)
	...
	function(a) {
		...
		b.prototype = {
			...
			resize: function(b, c) {
				...
				var d = this._();
				d.Combination || (d.width = d.push(d.W, b), d.height = d.push(d.H, c), d.size(d)), d.fireEvent(d, "resize", [b, c]), d.setUp(), d.plugins.eachAll(function(a) {
		...
		doConfig: function() {
			...
			if (b.items = [], b.labels = [], 0 == d) {
				...
				var k = c(i - j);
				if (h && c(h) < c(k)) e = c(Math.ceil(k / h));
				else {
					for (var l = k.toString(), m = l.indexOf(".") + 1, n = 10, o = (m > 0 ? /^0\.0*([1-9])$/ : /^([1-9])0*$/).exec(l); m > 0;) m--, n *= 10;
					o && o[1] && (e = parseInt(o[1])), e = 1 == e ? 5 : e, e = 8 == e || 2 == e ? 4 : e, e = 9 == e ? 3 : e, h = b.push("scale", k * n / e / n)
				}
				parseInt(h) != h && 0 == b.get("decimalsnum") && b.push("decimalsnum", (h + "").substring((h + "").indexOf(".") + 1).length)
			}
			b.push("distanceOne", b.get("valid_distance") / b.push("scale_share", e));
			var p, q, r, s = 0,
				t = 0,
				u = 0,
				v = 0,
			...
			for (var F = 0; e >= F; F++) p = d ? b.get("labels")[F] : (h * F + j).toFixed(b.get("decimalsnum")), q = b.isH ? b.get("valid_x") + F * b.get("distanceOne") : b.x, r = b.isH ? b.y : b.get("valid_y") + b.get("valid_distance") - F * b.get("distanceOne"), b.items.push({
				x: q,
				y: r,
				x0: q + u,
				y0: r + v,
				...
				originx: q + w,
				originy: r + x
			}, b.fireEvent(b, "parseText", [p, q + w, r + x, F, e == F]))), {
				textAlign: B,
				textBaseline: D
			}), b))
		}
		...
		getScaleObj: function(a, b) {
			for (var c = this._(), d = 0; d < c.scale.length; d++)
				if (c.scale[d].get("position") == a) return c.scale[d];
		...
		parse: function(b) {
			...
			c.each(function(c) {
				c.x_ = c.x, c.y_ = c.y, c.ignored || c.direct || !e || (b.push("label.originx", c.x), b.push("label.originy", c.y - l / 2 - 1), b.push("label.text", b.fireString(b, "parseText", [b, c.value], c.value)), a.applyIf(b.get("label"), {
					textBaseline: "bottom",
					color: b.get("f_color")
					...
					r && delete b.get("tip.listeners").parseText, b.mocker = new a.Custom({
						eventValid: function(a) {
							return l = b.lines[0].isEventValid(a), l.hit = k != l.i, l.valid && (k = l.i, j = [], b.lines.each(function(b, c) {
								m = b.isEventValid(a), 0 == c ? l.minTop = l.maxTop = m.top : (l.minTop = Math.min(l.minTop, m.top), l.maxTop = Math.max(l.maxTop, m.top)), j.push(r ? r(b, m.name, m.value, m.text, m.i) : m.name + " " + m.value)
		...
		doConfig: function() {
			...
			if (!b.isE()) {
				var c, d, e, f, g, h, i = b.coo.valid_height,
					...
					l = b.get("nullToDirect");
				b.push("sub_option.tip.showType", "follow"), b.push("sub_option.coordinate", b.coo), b.push("sub_option.tipInvokeHeap", b.tipInvokeHeap), b.push("sub_option.point_space", j), b.data.each(function(m) {
					c = b.coo.getScale(m.scaleAlign || b.get("scaleAlign")), g = b.get("sub_option.originy") - c.basic * i, d = [], m.value.each(function(n, o) {
						null != n ? (e = j * o, f = (n - c.start) * i / c.distance, h = {
							x: k + e,
							y: g - f,
							value: n,
							text: m.name + " " + n
						}, a.merge(h, b.fireEvent(b, "parsePoint", [m, n, e, f, o, c])), d.push(h)) : d.push({
							ignored: !l,
							direct: l
						})
